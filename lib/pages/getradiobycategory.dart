...
import '../model/getradiobycategorymodel.dart';

class GetRadioByCategory extends StatefulWidget {
  final String title, userid, categoryid;
  const GetRadioByCategory({
    super.key,
    required this.title,
  ...
  @override
  State<GetRadioByCategory> createState() => GetRadioByCategoryState();
}

class GetRadioByCategoryState extends State<GetRadioByCategory> {
  ...
  Future<List<Result>> fetchNewData(int? nextPage) async {
    final radiobycategoryprovider =
        Provider.of<GetRadiobyCategoryProvider>(context, listen: false);
    pageno = (nextPage ?? 0) + 1;
    debugPrint("Pageno =>$pageno");
    String languageid = await sharedPre.read("languageid");
    debugPrint("languageid =>$languageid");
    await radiobycategoryprovider.getRadiobycategory(
        widget.userid, widget.categoryid, languageid, pageno.toString());
    return radiobycategoryprovider.getradiobycategoryModel.result!;
  }
        ...
        Scaffold(
          ...
          body: Column(
            children: [
              ...
              Expanded(child: categoryradioList()),
            ],
          ),
        ),
  ...
  Widget categoryradioList() {
    return Consumer<GetRadiobyCategoryProvider>(
            ...
            return InkWell(
              onTap: () {
                playAudio(
                    ...
                    entry.id.toString(),
                    index,
                    radiobylanguageprovider.getradiobycategoryModel.result!
                        .toList());
              },
              child: Padding(
